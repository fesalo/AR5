...
<body>
    ...
    if ($_SERVER["REQUEST_METHOD"] == "POST") {
        ...
        for($i=0;$i<count($data);$i++){
            $foodFav=explode(", ",$data[$i]["Favorites"]);
            if(in_array("pasta",$foodFav)){
                $peopleFav++;
                $countriesFav[]=$data[$i]["Country"];
            }
        }
        $countriesFav=array_unique($countriesFav);
        $countriesFav=implode(",",$countriesFav);
        echo "Total people who likes $food: $peopleFav. <br>
        People's countries who likes $food: $countriesFav ";
    }
    ?>
    <form method="POST" action="">
        <label for="food">Comida:</label>
        <input type="text" id="food" name="food" value="<?php $food ?>">
        <br>
